...
impl UserMutation
{
	pub async fn create_new_user(db: &DbConn, form_data: user::RegisterUserModel) -> Result<user::ActiveModel, DbErr>
	{
	...
	pub async fn update_user_by_id(db: &DbConn, id: i32, form_data: user::Model) -> Result<user::Model, DbErr>
	{
		let user: user::ActiveModel = User::find_by_id(id).one(db).await?.ok_or(DbErr::Custom("Cannot find user.".to_owned())).map(Into::into)?;
