...
impl PostMutation
{
	...
	{
		post::ActiveModel { user_id: Set(form_data.user_id.to_owned()),
	...
	{
		let post: post::ActiveModel = Post::find_by_id(id).one(db).await?.ok_or(DbErr::Custom("Cannot find post.".to_owned())).map(Into::into)?;

		post::ActiveModel { id:      post.id,
		                    user_id: Set(form_data.user_id.to_owned()),
		                    title:   Set(form_data.title.to_owned()),
		                    text:    Set(form_data.text.to_owned()) }.update(db)
		                                                       .await
	}
