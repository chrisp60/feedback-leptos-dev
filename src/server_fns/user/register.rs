...
async fn check_inputs(
    ...
    if password.len() < 8 {
        return Err(ServerFnError::WrappedServerError(
            UserRegistrationError::PasswordTooShort,
        ));
    ...
    let day: u32 = date_of_birth.split("-").collect::<Vec<&str>>()[2]
        .parse::<u32>()
        .unwrap();
    let month: u32 = date_of_birth.split("-").collect::<Vec<&str>>()[1]
        .parse::<u32>()
        .unwrap();
    let year: i32 = date_of_birth.split("-").collect::<Vec<&str>>()[0]
        .parse::<i32>()
        .unwrap();
    ...
    match age.cmp(&18) {
        std::cmp::Ordering::Less => {
            return Err(ServerFnError::WrappedServerError(
                UserRegistrationError::InvalidDateOfBirth,
            ));
        }
    ...
    if first_name.len() < 2 || last_name.len() < 2 {
        return Err(ServerFnError::WrappedServerError(
            UserRegistrationError::InvalidNameLength,
        ));
    }
