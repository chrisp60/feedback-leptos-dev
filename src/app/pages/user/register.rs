...
fn Reg() -> impl IntoView {
    ...
    let err = Signal::derive(move || {
        (register_user_action.value())().map_or("".to_owned(), |result| match result {
            ...
            Err(err) => {
                ...
                if e.contains("Username") && e.contains("already in use") {
                    ...
                    return "Error:- Invalid Date of Birth, you must be 18 years or older to use \
                            this app!"
                        .to_string();
                } else if e.contains("Username") && e.contains("Invalid") {
                    return "Error:- Usernames must be between 6 and 30 characters long"
                        .to_string();
                } else if e.contains("Name") && e.contains("Length") {
                    return "Error:- First and last names must be at least 2 characters long"
                        .to_string();
                    ...
                    return "Error:- Password must contain at least 1 of these symbols (!@#$%^&*)"
                        .to_string();
